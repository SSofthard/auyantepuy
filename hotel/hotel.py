...
class hotel_room_type(osv.Model):
    ...
    _columns = {
        'cat_id': fields.many2one('product.category', 'Categoría', required=True, select=True, ondelete='cascade'),
    }
    _defaults = {
        'isroomtype': 1,
    }
...
#~ class product_template(osv.Model):
    ...
    #~ _columns = {
        #~ 'website_published_hotel':fields.boolean('Is Published Website'),
    #~ }
    #~ _defaults = {
        #~ 'website_published_hotel': 1,
    #~ } 
...
class hotel_room_amenities(osv.Model):
    ...
    _inherits = {'product.template': 'room_categ_id'}
    _columns = {
        'room_categ_id':fields.many2one('product.template', 'Product Category', required=True, ondelete='cascade'),
...
class hotel_room(osv.osv):
    ...
    def costos_total(self, cursor, user, ids, name, arg, context=None):
        ...
        for room in self.browse(cursor, user, ids, context=context):
            res[room.id] = round((room.list_price+(room.list_price*room.taxes_id.amount)),precision)
        return res
    _columns = {
